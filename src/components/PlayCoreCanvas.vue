...
import { run } from '@/play/run';
import { rgb2hex } from '@/play/modules/color.js';
import Canvas from '@/play/modules/canvas.js';
import Random from 'random-seed';
import MMCQ from 'node-vibrant/lib/quantizer/mmcq';

let rand;
let canvas;
let data;
const cgaColors = '#000000 #AA0000 #00AA00 #AA5500 #0000AA #AA00AA #00AAAA #AAAAAA #555555 #FF5555 #55FF55 #FFFF55 #5555FF #FF55FF #55FFFF #FFFFFF';
// const cgaColors = '#000000 #AA0000 #000000 #AA5500 #0000AA #AA00AA #00AAAA #000000 #000000 #FF5555 #55FF55 #FFFF55 #5555FF #FF55FF #55FFFF #000000';
const vgaColors = '#000000 #800000 #008000 #808000 #000080 #800080 #008080 #C0C0C0 #808080 #FF0000 #00FF00 #FFFF00 #0000FF #FF00FF #00FFFF #FFFFFF ' +
...
export default {
  ...
  mounted() {
    run({
      boot: this.setup,
      main: (c) => (this.$context) ? this.draw(c.y * this.cols * 2 + c.x) : null,
      settings: {
        element: this.$refs.play,
        once: true,
        cols: this.cols,
  ...
  watch: {
    ...
    cols() {
      this.$context.settings.cols = this.cols;
      this.setup();
      this.refresh();
    },
    rows() {
      this.$context.settings.rows = Math.floor(this.rows * 0.5);
      this.setup();
      this.refresh();
    },
    chars() {
  ...
  methods: {
    setup() {
      ...
      // Create our canvas to match our desired output.
      canvas = new Canvas().resize(this.cols, this.rows);

      const h = parseFloat(this.hue) || 0;
      const b = parseFloat(this.brightness) || 100;
      const c = parseFloat(this.contrast) || 100;
      const s = parseFloat(this.saturation) || 100;
      const i = parseFloat(this.invert) || 0;

      canvas.ctx.filter = `hue-rotate(${h}deg) brightness(${b}%) contrast(${c}%) saturate(${s}%) invert(${i}%)`;

      if (this.smoothing !== 'none') {
        canvas.ctx.imageSmoothingQuality = this.smoothing;
      ...
      canvas.fit(this.image);

      // reduce the colors.
      let colors;
      switch (this.quantize) {
        case 'count':
          colors = MMCQ(canvas.getImageData(), { colorCount: this.colorCount }).map(function (c) {
            return { r: c.r, g: c.g, b: c.b };
          });
          break;

        case 'palette':
          colors = this.palette.split(' ').map((hex) => this.hexToRgb(hex));
          break;

        case 'cga':
          colors = cgaColors.split(' ').map((hex, i) => {
            const c = this.hexToRgb(hex);
            return { r: c.r, g: c.g, b: c.b, c: i };
          });
          break;

        case 'vga':
          colors = vgaColors.split(' ').map((hex, i) => {
            const c = this.hexToRgb(hex);
            return { r: c.r, g: c.g, b: c.b, c: i };
          });
          break;
      ...
      if (colors) canvas.quantize(colors);

      canvas.process((i, pixel) => {
        pixel.char = this.chars[rand(this.chars.length)];
    ...
    refresh() {
      requestAnimationFrame(this.$context.callback);
    },
    draw(i) {
      let bg = data[i];
      let fg = data[i + this.cols];
      if (!fg) return false;

      return { char: bg.char, color: fg.hex, backgroundColor: bg.hex };
    },
    hexToRgb(hex) {
